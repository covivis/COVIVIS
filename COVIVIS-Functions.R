...
#back projection of disease onset counts from the number of reported cases using Monte Carlo method
...
# V-I and I-V parameters estimation by linear regression model
...
# VI and IV predictions of disease onset counts by linear regression model
epi.prediction_by_lm <- function(xdata,pa,pb,pr){
  ...
  # predicted number of onsets
  prediction.y <- unlist( lapply(xdata[,2],est_value) )
  prediction.y <- log10(prediction.y) 
...
vi.param_estim_by_em_rev <- function(sewagedata,onsetdata){
...
vi.prediction_by_em_rev <- function(sewagedata,v,omega,gamma,pr){
...
iv.param_estim_by_em <- function(onsetdata,sewagedata){
  ...
  z0 <- c(10, 1, 1) #( v, omega, gamma) initial value
